...
Edge::Edge(Node *in_pred_node, Node *in_succ_node)
{
	...
	else
	{
		...
		if ( Edge::succ_node->succ_edge == NULL )
		{
			Edge::succ_node->succ_edge = this;
			Edge::next_succ_edge = NULL;
			Edge::prev_succ_edge = NULL;
		}
		else
		{
			Edge::next_succ_edge = succ_node->succ_edge;
			Edge::prev_succ_edge = NULL;
			Edge::succ_node->succ_edge->next_succ_edge = this;
			Edge::succ_node->succ_edge = this;
		}

		if ( Edge::pred_node->pred_edge == NULL )
		{
			Edge::pred_node->pred_edge = this;
			Edge::next_pred_edge = NULL;
			Edge::prev_pred_edge = NULL;
		}
		else
		{
			Edge::next_pred_edge = pred_node->pred_edge;
			Edge::prev_pred_edge = NULL;
			Edge::pred_node->pred_edge->prev_pred_edge = this;
			Edge::pred_node->pred_edge = this;
		}
	}
}
